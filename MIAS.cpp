...
\version   1.0
\date      2020-12-03
\author    Jing Ning @ Sun Lab, Westlake University
...
//-------------------------global-------------------------
#define USE_BUFFER 
#define CAM_PER_TYPE 10
#define MSG_QUEUE_NUM CAM_PER_TYPE*8

int g_multi_part = 3;
int g_buffer_frames = 1500;
...
public:
	...
	void open_log() {
		time_t t = time(0);
		strftime(g_tmp, sizeof(g_tmp), "%Y%m%d_%H%M%S", localtime(&t));
		sprintf(log_name, "%s_%s", g_tmp, cam_name);
		create_dir("avi"); // create if not exists
		sprintf(log_dir, "avi/%s", log_name);
		create_dir(log_dir);
		sprintf(g_tmp, "avi/%s/%s.log", log_name, log_name);
		log.open(g_tmp, ios::out);
	}
	void close_log() {
		if (log.is_open()) {
	...
	bool begin_store_avi(const char* fourcc, float fps = 100) {
		...
		char file_name[1000];
		sprintf(file_name, "avi/%s/%s.avi", log_name, log_name);
		vw.open(file_name, CV_FOURCC(fourcc[0], fourcc[1], fourcc[2], fourcc[3]), fps, cvSize(image_width, image_height), is_color);
		uc = is_color ? CV_8UC3 : CV_8UC1;
		data_size = image_height * image_width;
		log << "file:\t\t\t" << file_name << endl;
		if (vw.isOpened())
			log << "open:\t\t\t" << get_time_str(g_tmp) << endl;
		else {
			log << "open video failed:\t\t\t" << get_time_str(g_tmp) << endl; //need opencv_ffmpeg342_64.dll
	...
	double get_ts_inc(uint64_t ts) {
		if (init_ts == 0) {
			...
			log << "start:\t\t\t" << get_time_str(g_tmp) << endl;
		}
	...
	int process_raw_image(const void* pBuffer, int64_t ts=0) {
		...
		if (!put_msg(msg, idx)) {
			...
			//}
			//else {
			//	cout << "[" << msg->pszName << "]" << "lost " << float(lost/fps) << endl;
			//	lost = 0;
			//}
		}
#else
...
public:
	int nDeviceNum = 0;
	CAMERA_INFO mCams[CAM_PER_TYPE];
	const char* fourcc;
	bool need_record;
	Json::Value *pConfig;
	int frame_seq;
	...
	void init_one_cam(int cam_idx, int camera_type_id, bool need_record, const char* _serial_no, Json::Value* pConfig, int width, int height, const char* fourcc, float fps) {
		if (g_multi_part > 1) {
			for (int i = 0; i < g_multi_part; i++) {
				sprintf(g_tmp, "%s_%d", _serial_no, i);
				mCams[i].init_cam(i, need_record, g_tmp);
				//mCams[i].first_time_init(pConfig, width, height, fourcc, fps);
			}
		}
		else {
	...
	void process_one_cam(int cam_idx, const void* pBuffer, int64_t ts = 0){
		if (g_multi_part > 1) {
			mCams[frame_seq % g_multi_part].process_raw_image(pBuffer, ts);
			frame_seq++;
		}
		else {
	...
	int do_open() {
		try {
			...
			if (tlFactory.EnumerateDevices(devices) == 0) {
				cout << "[Pylon] deviceNum=0" << endl;
				return 0;
			}
			nDeviceNum = devices.size();
			cout << "[Pylon] deviceNum=" << nDeviceNum << endl;
			...
			for (int i = 0; i < nDeviceNum; i++) {
				...
				cout << "[Pylon] open " << name << endl;
			}
			cameras.StartGrabbing();
		}
		catch (const GenericException &e) {
			cerr << "[Pylon] An exception occurred." << endl << e.GetDescription() << endl;
			return 0;
		}
		return 0;
	}
...
public:
	...
	int do_open() {
		...
		cout << "[PointGrey] deviceNum=" << nDeviceNum << endl;
		if (nDeviceNum <= 0) return 0;
		try {
			for (int i = 0; i < nDeviceNum; i++) {
				if (g_only_one_camera && i != 0) {
					mCams[i].is_open = false;
					cout << "[PointGrey] skip " << endl;
					continue;
				}
				...
				cout << "[PointGrey] open " << deviceSerialNumber << endl;
			...
			cerr << "[PointGrey] An exception occurred." << endl << e.what() << endl;
		}
		return 0;
	}
	int do_capture() {
	...
	void one_frame_func(int i) {
		...
		if (pResultImage->IsIncomplete()) {
			cout << "[PointGrey] Image incomplete: " << Spinnaker::Image::GetImageStatusDescription(pResultImage->GetImageStatus()) << endl;
			return;
		}
...
void* write_thread(void *arg) {
	...
	cout << "write_thread" << idx << " end " << pCam->cam_name << endl;
